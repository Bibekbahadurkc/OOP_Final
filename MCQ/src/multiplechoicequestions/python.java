...
public class python extends JFrame implements ActionListener{
     JLabel Equestion,PointA,PointB,PointC,PointD,Ranswer;
     JTextField Equestionbox,Ans1,Ans2,Ans3,Ans4,RAnswerbox; 
     JButton EBack,ESave;
     
    public python(){
     
  Equestion= new JLabel("Question");
  Equestion.setBounds(50,50,120,40);
  add(Equestion);
  
 Equestionbox = new JTextField();
 Equestionbox.setBounds(115,50,350,45);
 add(Equestionbox);
 ...
 EBack.setBounds(50, 450, 150, 35);
 add(EBack);
 EBack.addActionListener(this);
 
 ESave = new JButton("Save");
 ESave.setBounds(400,450,150,35);
 add(ESave);
 ...
 c.setLayout(null);
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if(ae.getSource()==EBack){
            new module().setVisible(true);}
        
        if(ae.getSource()==ESave){
         try {
                database db=new database();
                int result=db.ethicsSave(Equestionbox.getText(),Ans1.getText(),Ans2.getText(),Ans3.getText(),Ans4.getText(),RAnswerbox.getText());
                if(result>0)
